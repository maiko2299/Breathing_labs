...
export const CustomBuilder: React.FC<{ 
  ...
  const [description, setDescription] = useState("A personalized breathing pattern.");
  const [phases, setPhases] = useState<{ phase: BreathingPhase; durationSecs: number }[]>([
    { phase: "inhale", durationSecs: 4 }
  ]);

  const addPhase = (phase: BreathingPhase) => {
    setPhases([...phases, { phase, durationSecs: 4 }]);
  };
  ...
  return (
    <div className="w-full max-w-2xl mx-auto space-y-8 animate-in fade-in zoom-in-95 duration-500">
      <div className="text-center space-y-2 mb-8">
        <h2 className="text-3xl font-light tracking-tight text-primary-light">Custom Protocol</h2>
      ...
      <Card className="space-y-6">
        ...
        <div className="space-y-4 mt-6">
          ...
          {phases.map((p, i) => (
            <div key={i} className="flex items-center space-x-4 bg-black/10 p-3 rounded-lg border border-card-border/50">
      ...
      <div className="flex justify-between pt-4">
        ...
        <Button onClick={handleSave}>Save & Use Protocol</Button>
      </div>
    </div>
  );
};
